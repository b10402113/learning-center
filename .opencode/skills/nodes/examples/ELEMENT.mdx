...
Complete `learn/<subject>/elements/<element-id>.mdx` files as they should look after `/nodes`. Each example shows one element `type` with its frontmatter, body sections, glosses, cross-element `Connections`, source-linked `Deep dive`, and retrieval `Questions`, in the subject's `language`.
