...
5. **Write.** After the learner passes, compose the `## Lesson` body as a teach-node-style MDX article. Use the segmented writing protocol to fill `## Lesson` in the skeleton via `edit` — split long bodies into 2–3 paragraphs. The MDX lesson is the long-term record; the HTML is the reading experience. Append a `## Course` section after `## Lesson` with the relative path to the HTML lesson: `## Course\n\n[↗ 課程頁面](../../lessons/<step-id>.html)` — this lets the learner jump from the MDX article to the interactive HTML lesson.
