...
Start a new learning path. The subject **comes from the argument** — `sources/<subject>/` must exist. If no argument is given, ask for it. Before any roadmap can be built you must know the learner — every node anchors to their experience, so the profile is the foundation.
...
- **Goal** — what they want to *be able to do* when this path is done. Concrete, not "understand X": "ship a production MCP server", "read retrieval papers".
- **Why** — the motivation underneath. The anchor that carries the path when it gets hard.
- **Prior experience** — what they already know that this subject can build on.
- **Anchors** — concrete experiences the nodes can connect to. Push for real specifics ("I run a Rails app", "I've used VS Code"), not "some backend".
...
- **Language** — what language every generated article for this subject should be in (e.g. `en`, `zh-Hant`). This is a hard commitment: later paths, nodes, and edges are written in this language.
...
- **Tune scope** — which artifacts get the voice: `paths` (lesson narratives only), `nodes-paths` (lesson plus node prose, sections kept), or `all` (also edges). Default `nodes-paths`.
...
tune-scope: <paths | nodes-paths | all>
created: YYYY-MM-DD
updated: YYYY-MM-DD
---
...
## Goal
<what the path should let them do>
...
## Anchors
- <a concrete experience a node can link to>
- ...
