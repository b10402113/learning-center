...
- `/to-article <subject> [tier<N> | node-slug ...]` — one `article-agent` per scoped step: `src/html.mjs` makes exactly one `.env` text-model call to rewrite the HTML lesson into a fluent article (validating that code blocks and links survive), then the agent reads the finished article, inserts `<!--image:N-->` markers where figures should go, writes the plan, and a program-only finalize step validates the markers, turns them into `<figure>` placeholders and records the plan under `learn/<subject>/output/<node>/<step>/`. No images are generated; step frontmatter moves to `illustration: planned`.
- `/to-image <subject> [tier<N> | node-slug ...]` — one `article-agent` per scoped step runs `src/html.mjs` to generate each planned figure into `lessons/<node>/<step>-assets/`, swap the placeholders for `<figure>`, and set `illustration: done`. Spends image-API money, so it confirms first unless `-skip-ask`.
