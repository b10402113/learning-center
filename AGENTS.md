...
- `/to-article <subject> [tier<N> | node-slug ...]` — one `article-agent` per scoped step: it decides the image plan, then `src/html.mjs` makes exactly one `.env` text-model call to rewrite the HTML lesson into an illustrated article (validating that code blocks and links survive and that every image marker is present and in order) and records the plan under `learn/<subject>/output/<node>/<step>/`. No images are generated; step frontmatter moves to `illustration: planned`.
