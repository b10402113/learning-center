...
source-reading 協定擴展：codebase digest 的 L1 為「架構總覽」（entry point、核心目錄、模組圖），L2 為「關鍵檔案 + 摘要 + 檔案 locator」；仍遵循「不可變來源 → digest」兩層契約與 `source_hash` 生命週期。大小閾值改用「檔案數 × 平均大小」而非 pdftotext 行數。path 計數基準線對 codebase 來源改用結構化 metric（模組數／檔案數），不硬套 `lines/1100`。
...
- [ ] codebase digest 格式（L1 架構總覽、L2 關鍵檔案 + locator）與 `source_hash` 生命週期
